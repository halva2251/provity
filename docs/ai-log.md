...
| 2026-06-21 | Claude | Deployment auf Vercel (CLI, Root `app/`, Env-Variablen, Prod-Deploy) + PWA installierbar machen (Web-Manifest, Icons, theme-color; Manifest-Route aus Auth-Middleware ausschliessen) | Vercel (`provity.vercel.app`), `app/src/app/manifest.ts`, `app/src/app/layout.tsx`, `app/public/icon-*.png`, `app/src/proxy.ts` |
| 2026-06-21 | Claude | Pomodoro Arbeits-/Pausen-Intervall (25/5 min, Intervallwechsel laut Projektauftrag Ziel 3): Phasen-Logik im Timer, Phase in localStorage persistiert (rückwärtskompatibel), Dashboard-Widget zeigt Phase, nur Arbeits-Sessions zählen; Entscheidung dokumentiert | `app/src/lib/pomodoro.ts`, `app/src/lib/pomodoroTimer.ts`, `app/src/app/pomodoro/Timer.tsx`, `app/src/app/pomodoro/page.tsx`, `app/src/app/dashboard/PomodoroLiveStatus.tsx`, `docs/entscheidungen.md` |
